...
impl<B: BlockchainNetwork> MulticallData<B> for ReservesCallData<B> {
    ...
    fn to_calls(&self) -> Self::Calls {
        let slot0_call = contract::getSlot0Call {
            poolId: self.pool_id,
        };

        let slot0_call_data = slot0_call.abi_encode();

        let liquidity_call = contract::getLiquidityCall {
            poolId: self.pool_id,
        };
    ...
    fn decode_output(&self, response: &[Bytes]) -> Result<Self::Output> {
        ...
        if let Some(_) = response.get(2) {
            ...
            let slot_0_output = contract::getSlot0Call::abi_decode_returns(slot_0_bytes)
                .map_err(|e| anyhow!("slot0call decode failed: {}", e))?;
            let liquidity_output = contract::getLiquidityCall::abi_decode_returns(liquidity_bytes)
