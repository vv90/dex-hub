...
impl std::fmt::Display for ReservesCallDataDecodeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "Failed to decode reserves call data for {} Pancakeswap V3 pool {}: {}",
            blockchain.name(),
            address,
            self.message
        )
            ...
            Ok((
                ...
                VirtualReserves {
                    token0: try_into_decimal(reserve0, self.token0_decimals).map_err(|e| {
                        self.decode_error(format!("Failed to convert token0 reserve: {}", e))
                    })?,
                    token1: try_into_decimal(reserve1, self.token1_decimals).map_err(|e| {
                        self.decode_error(format!("Failed to convert token1 reserve: {}", e))
                    })?,
                    max_swap0: try_into_decimal(max_swap0, self.token0_decimals).map_err(|e| {
                        self.decode_error(format!("Failed to convert max_swap0: {}", e))
                    })?,
                    max_swap1: try_into_decimal(max_swap1, self.token1_decimals).map_err(|e| {
                        self.decode_error(format!("Failed to convert max_swap1: {}", e))
                    })?,
